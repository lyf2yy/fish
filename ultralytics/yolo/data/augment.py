...
class AddGaussianNoise:
    ...
    def __init__(self, mean=0.0, variance=1.0, amplitude=20.0, p=0.2):
...
class RandomLum:
    ...
    def __init__(self, p=0.5, gamma=None) -> None:
        self.p = p
        if gamma:
            self.gammas = gamma
        else:
            self.gammas = [0.7,0.8, 0.9, 1.1, 1.2,1.3]

    def __call__(self, labels):

        # 图片是灰度图,则以概率p变换灰度
        if random.random() < self.p and labels['img'].shape[-1] == 1:
            gamma =  random.choice(self.gammas)
                
            img = labels['img']
            img = np.array(img, dtype=np.uint8)
            meansValue = np.mean(img)
            if meansValue < 65:
                gamma = random.choice(self.gammas[0:3])
            if meansValue > 155:
                gamma = random.choice(self.gammas[3:6])
            gamma_table = [np.power(x, gamma).clip(0,255) for x in range(256)]# 建立映射表
            gamma_table = np.round(np.array(gamma_table)).astype(np.uint8)# 颜色值为整数
            
            img = cv2.LUT(img, gamma_table)# 图片颜色查表。另外可以根据光强（颜色）均匀化原则设计自适应算法。
            img = img.astype(np.float64) # uint8 to fp16/32
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        r = min(new_shape[0] / shape[0], new_shape[1] / shape[1])#将长边变为new_shape所需要的倍数
...
class RandomHightCrop:
    # YOLOv8 CenterCrop class for image preprocessing, i.e. T.Compose([CenterCrop(size), ToTensor()])
    def __init__(self, size=640, p=0.5, rate=0.2, single_channel=True):
        super().__init__()
        self.h, self.w = (size, size) if isinstance(size, int) else size
        self.imgsz = size
        self.p = p
        self.single_channel = single_channel
        self.rate = rate #默认最大裁剪比例

    def loadImg(self, im_file):
    ...
    def __call__(self, labels):
        ...
        if random.random() < self.p :                
            # print(labels.keys())
            img = self.loadImg(labels['im_file'])#读取原始图片
            ht, wt, _ = img.shape
            if ht / wt < 3:
                labels
            hs, ws = labels['ori_shape']
            instance = labels['instances']
            randomRate = random.random() * self.rate
            ymin = int(hs * randomRate)
            ymax = hs - ymin
            img = img[ymin:ymax, ...]
            labels['ori_shape'] = img.shape[:2]
            
            instance.CenterCrop(0, ws, ymin, ymax)
...
def v8_transforms(dataset, imgsz, hyp):
    pre_transform = Compose([
        AddGaussianNoise(mean=hyp.Noisemean, variance=hyp.Noisevariance, amplitude=hyp.Noiseamplitude, p=hyp.Noisep),
        RandomLum(p=hyp.Lumenchance),#灰度值变换
        RandomHightCrop(imgsz, p=hyp.HeightCrop, single_channel = hyp.single_channel),#对高度值随机裁剪，不超过box，没有box的话就取5%-20%的范围，取一个随即大小
        RandomFlip(direction='vertical', p=hyp.flipud),
        RandomFlip(direction='horizontal', p=hyp.fliplr),
        Mosaic(dataset, imgsz=imgsz, p=hyp.mosaic, border=[-imgsz // 2, -imgsz // 2]),
        CopyPaste(p=hyp.copy_paste),
        RandomPerspective(#投射变换
            degrees=hyp.degrees,

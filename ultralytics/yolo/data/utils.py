...
def get_hash(paths):
    ...
    size = sum(os.path.getsize(p) for p in paths if os.path.exists(p))  # 文件大小
...
def verify_image_label(args):
    ...
    try:
        ...
        assert (shape[0] > 9) & (shape[1] > 9), f'image size {shape} <10 pixels' #圖像最小的長寬
        assert im.format.lower() in IMG_FORMATS, f'invalid image format {im.format}'#後綴格式
...
def check_det_dataset(dataset, autodownload=True):
    ...
    # Checks
    for k in 'train', 'val', 'names':#配置文件中train, val, names是不是缺失
        if k not in data:
            raise SyntaxError(
                emojis(f"{dataset} '{k}:' key missing ❌.\n"
                       f"'train', 'val' and 'names' are required in data.yaml files."))
    if isinstance(data['names'], (list, tuple)):  # old array format， class name转换为字典
        data['names'] = dict(enumerate(data['names']))  # convert to dict
    data['nc'] = len(data['names'])#num_class
    ...
    if val:
        val = [Path(x).resolve() for x in (val if isinstance(val, list) else [val])]  # val path #根据系统转换为绝对路径

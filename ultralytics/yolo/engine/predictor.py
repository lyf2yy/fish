...
class BasePredictor:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None):
        ...
        self.save_dir = increment_path(Path(project) / name, exist_ok=self.args.exist_ok) #log文件路径，如果已经存在，序号自增
        if self.args.conf is None:#置信度阈值缺省为0.25
            self.args.conf = 0.25  # default conf=0.25
        self.done_warmup = False
        if self.args.show:#是否支持显示图像
    ...
    def setup_source(self, source):
        ...
        self.dataset = load_inference_source(source=source,
                                             ...
                                             auto=self.model.pt,
                                             single_channel=self.args.single_channel)

...
class Instances:
    ...
    def CenterCrop(self, begin_w, end_w, begin_h, end_h):
        # 图像中心裁剪,box的坐标减去w和h
        ori_format = self._bboxes.format
        self.convert_bbox(format='xyxy')
        self.bboxes[:, 0] = self.bboxes[:, 0] - begin_w
        self.bboxes[:, 1] = self.bboxes[:, 1] - begin_h
        self.bboxes[:, 2] = self.bboxes[:, 2] - begin_w
        self.bboxes[:, 3] = self.bboxes[:, 3] - begin_h

        if ori_format != 'xyxy':
            self.convert_bbox(format=ori_format)
        self.segments[..., 0] = self.segments[..., 0] - begin_w
        self.segments[..., 1] = self.segments[..., 1] - begin_h
        if self.keypoints is not None:
            self.keypoints[..., 0] = self.keypoints[..., 0] - begin_w
            self.keypoints[..., 1] = self.keypoints[..., 1] - begin_h
        self.clip(end_w, end_h)

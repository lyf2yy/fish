...
def one_cycle(y1=0.0, y2=1.0, steps=20):#正弦曲线衰减
...
def init_seeds(seed=0, deterministic=False):
    ...
    if deterministic and TORCH_1_12:  # https://github.com/ultralytics/yolov5/pull/8213
        torch.use_deterministic_algorithms(True) #每次的卷积使用固定算法，如果为false会自动选择最快的算法，但是结果可能有差异，不一定能复现
        torch.backends.cudnn.deterministic = True
        os.environ['CUBLAS_WORKSPACE_CONFIG'] = ':4096:8' #cuBLAS 库 随机种子#使用 CUDA Toolkit 10.2 或更高版本构建 cuDNN 库时，cuBLAS 库中新的缓冲区管理和启发式算法会带来随机性。
